...
void Error::print_file_table() {
    ...
    for (uint16_t i = 0; i < n_files; i++) {
        SER.printlnf(Serial, "  -%I16u: %s", i, file_names[i]);
    }
}
...
void Error::_throw(err_t error, uint16_t file_id, uint32_t line) {
    SER.printlnf(Serial, "Error #%I32u thrown in file %s and line %I32u.", (uint32_t)error, file_names[file_id], line);
}

void Error::_rethrow(err_t error, uint16_t file_id, uint32_t line) {
    SER.printlnf(Serial, "Error #%I32u re-thrown in file %s and line %I32u.", (uint32_t)error, file_names[file_id], line);
}

void Error::_capture(err_t error, uint16_t file_id, uint32_t line) {
    SER.printlnf(Serial, "Error #%I32u captured in file %s and line %I32u.", (uint32_t)error, file_names[file_id], line);
}

// Acá entra cuando falla un assert
void aFailed(uint8_t* file, uint32_t line) {
    while (1) {
        SER.printf(Serial, "ERROR: Assert failed on file %s and line %I32u\n", file, line);
        delay(1000);
    }
}